    ...
    methods: {
                ...
                for (let j = 0; j < 7; j++) {
                    if (i === 0 && j < firstDay) {
                        ...
                        let cellText =
                            ...
                            1;
                        let cell =
                            "<td class='prev-month month-day'><p class='date'>" +
                            cellText +
                            "</p></td>";
                        row += cell;
                    } else if (
                        ...
                        let cell =
                            "<td class='month-day'>" + cellText + "</td>";
                        ...
                        if (
                            ...
                            cell =
                                "<td class='month-day current-day'>" +
                                cellText +
                                "</td>";
                        }
                        row += cell;
        ...
        checkEventOnDate: function (date, month, year) {
            ...
            for (let value in this.events) {
                if (
                    new Date(this.events[value].start_datum).getMonth() ===
                        month &&
                    new Date(this.events[value].start_datum).getFullYear() ===
                        year
                ) {
                    if (this.events[value].end_datum === undefined) {
                        if (
                            new Date(
                                this.events[value].start_datum,
                            ).getDate() === date
                        ) {
                            badgeNumber++;
                            eIds.push(value);
                            if (!tableString.includes("eventCal"))
                                tableString +=
                                    "<div class='eventCal oneDay' @click='reroute(" +
                                    this.events[value].oaId +
                                    ")'><p class='eventTitle'>" +
                                    this.events[value].titel +
                                    "</p></div>";
                        }
                    } else {
                        if (
                            ...
                            (new Date(this.events[value].end_datum).getDate() >=
                                date ||
                                new Date(
                                    this.events[value].end_datum,
                                ).getMonth() >= month)
                        ) {
                            badgeNumber++;
                            eIds.push(value);
                            if (!tableString.includes("eventCal"))
                                tableString +=
                                    "<div class='eventCal oneDay flex flex-center' @click='reroute(" +
                                    this.events[value].oaId +
                                    ")'><p class='eventTitle'>" +
                                    this.events[value].titel +
                                    "</p></div>";
                    ...
                    new Date(this.events[value].start_datum).getMonth() <
                        month &&
                    new Date(this.events[value].end_datum).getMonth() > month
                ) {
                    badgeNumber++;
                    eIds.push(value);
                    if (!tableString.includes("eventCal"))
                    ...
                    if (
                        new Date(this.events[value].end_datum).getDate() >= date
                    ) {
                        badgeNumber++;
                        eIds.push(value);
                        if (!tableString.includes("eventCal"))
            ...
            if (badgeNumber > 0) {
                tableString +=
                    "<div class='badgeNumber' @click='toggle(" +
                    date +
                    ")'><p>" +
                    badgeNumber +
                    "</p></div>";
        ...
        createPopUp: function (array, date, month, year) {
            let modal =
                "<div class='modal flex flex-center' id='calPopUp" +
                date +
                "' style='display: none;'>";
            let content = "<div class='content flex flex-center'>";

            let close =
                "<div class='close row' id='close" +
                date +
                "' @click='toggle(" +
                date +
                ")'> x </div>";
            ...
            for (let id of array) {
                let row =
                    "<div class='row calRow' @click='reroute(" +
                    this.events[id].oaId +
                    ")'><p>";
